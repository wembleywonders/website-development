...
// ============================================
// CHILD INFO (for displaying active entity)
// ============================================
...
const MayaMessageBubble: React.FC<MayaMessageBubbleProps> = ({ message, onResponse }) => {
  ...
  const entityInfo = message.metadata?.childId 
    ? ENTITY_INFO[message.metadata.childId] 
    : ENTITY_INFO.maya;
  ...
  return (
    <div className={`${styles.messageBubble} ${styles[message.type]} ${getTypeClass()}`}>
      {/* Show entity indicator if not Maya */}
...
const MayaChatPanel: React.FC<MayaChatPanelProps> = ({
  ...
  // Different placeholder based on mode and entity
  const getPlaceholder = () => {
    if (activeEntity !== 'maya') {
      return `Message ${entityInfo.name}...`;
    }
    switch (mode) {
      case 'ACTIVE':
      ...
      case 'ROUTING':
        return 'Where do you need help?';
      default:
        return 'Type a message...';
    }
  };
  
  // Get role text for header
  const getRoleText = () => {
    if (activeEntity !== 'maya') {
      return entityInfo.role;
    }
    const stageDef = STAGE_DEFINITIONS[stage as 1|2|3|4|5];
    return stageDef?.mayaRole || 'Guide';
  };
      ...
      <div className={styles.stageIndicator}>
        ...
        <div className={styles.stageDots}>
          {[1, 2, 3, 4, 5].map(s => (
            <span 
              key={s} 
              className={`${styles.stageDot} ${s <= stage ? styles.active : ''}`}
            />
          ))}
        </div>
      </div>
...
export const MayaCompanion: React.FC<MayaCompanionProps> = ({
  ...
  const { messages, addMessage, clearMessages } = useMayaMessages();
  ...
  // Get silent observations from store
  ...
  // Set unread when new messages arrive while collapsed
  useEffect(() => {
  ...
  const handleSendMessage = (text: string) => {
    // Generate response based on active entity and mode
    const entityInfo = ENTITY_INFO[activeEntity];
    
    if (activeEntity === 'maya') {
      // Maya's responses based on mode
      if (currentMode === 'WITNESS' || currentMode === 'PARTNER') {
        setTimeout(() => {
          addMessage("What made you think of that?", 'reflection');
        }, 500);
      } else {
        setTimeout(() => {
          addMessage(
      ...
      // Child's responses - simple placeholder
      // In production, this would call the LLM with the child's personality
      setTimeout(() => {
        ...
        const entityResponses = responses[activeEntity] || responses.maya;
        const response = entityResponses[Math.floor(Math.random() * entityResponses.length)];
        addMessage(response, 'narration', { childId: activeEntity });
      }, 600);
    }
  };
  ...
  return (
    <div className={`${styles.mayaCompanion} ${styles[position]} ${className}`}>
      {/* Collapsed icon (WITNESS/PARTNER mode or when not expanded) */}
      {!isExpanded && (
        <MayaCollapsedIcon 
...
export const MayaContextPanel: React.FC<MayaContextPanelProps> = ({
  ...
  if (currentMode !== 'ACTIVE' || !preferences.showHints) {
    return null;
  }
  ...
  const getContextHint = () => {
    ...
    if (currentAction === 'stuck') {
      return "Being stuck is part of building. What have you tried so far?";
    }
    return null;
  };
...
export const MayaStatusIndicator: React.FC<MayaStatusIndicatorProps> = ({
  ...
  // Only show conversion prompt in WITNESS/PARTNER mode after work threshold
...
// ============================================
// CHILD QUICK SWITCH (New Component)
// ============================================
...
export const ChildQuickSwitch: React.FC<ChildQuickSwitchProps> = ({
  ...
  return (
    <div className={`${styles.quickSwitch} ${className}`}>
      {Object.entries(ENTITY_INFO).map(([id, info]) => (
        <button
          key={id}
          className={`${styles.quickSwitchBtn} ${id === currentEntity ? styles.active : ''}`}
          onClick={() => onSelect(id as ActiveChild)}
