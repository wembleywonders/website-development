...
import { ROVPersonality, ROVMessage, LearnerContext } from '../../learning-support/LearningROVSystem';
...
export const PathfinderROV: React.FC<PathfinderProps> = ({
  ...
  const generateSuggestions = (context: LearnerContext): string[] => {
    ...
    if (context.currentProgramme) {
      const progress = context.badgesEarned?.filter(
        b => b.startsWith(context.currentProgramme?.substring(0, 2).toLowerCase() || '')
      ).length || 0;
