...
1. **Broad consolidation — 🟢 ALL FOUR SUB-BUCKETS LANDED, 12 Sep 2026
   (Claude Code).** Landed across four branches (sandboxes/rov-toolbar/
   systems-rovs/rov-index, programme-journeys/pages/activities, types +
   tooling, and the docs cluster) rather than one giant commit — each
   file individually checked for real-vs-stale/orphan status and
   verified against the actual tsc baseline, not copied wholesale. Full
   rationale for every file, every deferred item, and every
   newly-surfaced (not newly-caused) error is in each branch's own
   commit messages; short version below.
